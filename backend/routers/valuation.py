...
async def calculate_dcf(inputs: DCFInputs) -> Dict:
    ...
    try:
        ...
        if not info or "symbol" not in info:
            raise HTTPException(
                status_code=404, detail=f"Stock {inputs.symbol} not found"
            )
        ...
        if financials.empty:
            raise HTTPException(
                status_code=400,
                detail="No financial data available for DCF calculation",
            )
        ...
        if latest_revenue <= 0:
            raise HTTPException(
                status_code=400, detail="Invalid revenue data for DCF calculation"
            )
        ...
        cash = (
            ...
            and "Cash And Cash Equivalents" in balance_sheet.index
            else 0
        )
        debt = (
        ...
        if shares_outstanding <= 0:
            raise HTTPException(
                status_code=400, detail="Invalid shares outstanding data"
            )
        ...
        upside_downside = (
            ((dcf_value_per_share - current_price) / current_price * 100)
            if current_price > 0
            else 0
        )
    ...
    try:
        ...
        return {
            ...
            "relative_valuation": {
                metric: (
                    "Undervalued"
                    if company_metrics.get(metric, 0) < industry_averages.get(metric, 0)
                    else "Overvalued"
                )
